...
let connectionString = process.env.DATABASE_URL || 'postgres://localhost:5432/sales'
...
export const getProductsByDescription = (req, res, next) => {
    ...
    db.any('select * from product where description ilike $1', `%${productDescription}%`)
        .then(data => {
            res.status(200)
                .json({
                    status: 'success',
...
export const createProduct = (req, res, next) => {
    ...
    db.one('insert into product(description, price, product_status_id)' +
...
export const updateProduct = (req, res, next) => {
    db.none('update product set description=$1, price=$2, product_status_id=$3 where product_id=$4',
    [req.body.description, parseFloat(req.body.price), parseInt(req.body.product_status_id), parseInt(req.body.product_id)])
        .then(() => {
            res.status(200)
                .json({
                    status: 'success',
...
export const createAccount = (req, res, next) => {
    ...
    db.one('insert into account(role_id, account_status_id)' +
...
export const updateAccount = (req, res, next) => {
    db.none('update account set role_id=$1, account_status_id=$2 where account_id=$3',
    [parseInt(req.body.role_id), parseInt(req.body.account_status_id), parseInt(req.body.account_id)])
        .then(() => {
            res.status(200)
                .json({
                    status: 'success',
...
export const getAllSalesOrders = (req, res, next) => {
    db.any('select sales_order_id, orders.sales_order_status_id, sales_order_status_name, created_at,' +
...
export const getSalesOrderByID = (req, res, next) => {
    ...
    db.one('select sales_order_id, orders.sales_order_status_id, sales_order_status_name, created_at,' +
            ' customer_id, customer_name, account_id, total from sales_order as orders' +
            ' join sales_order_status as status' +
            ' on status.sales_order_status_id = orders.sales_order_status_id' +
            ' where orders.sales_order_id = $1', salesOrderID)
        .then(data => {
            res.status(200)
                .json({
                    status: 'success',
...
export const getSalesOrdersByCustomerID = (req, res, next) => {
    ...
    db.any('select sales_order_id, orders.sales_order_status_id, sales_order_status_name, created_at,' +
            ...
            ' join sales_order_status as status' +
            ' on status.sales_order_status_id = orders.sales_order_status_id' +
            ' where orders.customer_id = $1', customerID)
        .then(data => {
            res.status(200)
                .json({
                    status: 'success',
...
export const getSalesOrdersByCustomerName = (req, res, next) => {
    ...
    db.any('select sales_order_id, orders.sales_order_status_id, sales_order_status_name, created_at,' +
            ...
            ' join sales_order_status as status' +
            ' on status.sales_order_status_id = orders.sales_order_status_id' +
            ' where orders.customer_name ilike $1', `%${customerName}%`)
        .then(data => {
            res.status(200)
                .json({
                    status: 'success',
...
export const createSalesOrder = (req, res, next) => {
    ...
    db.one('insert into sales_order(customer_id, customer_name, account_id, sales_order_status_id)' +
...
export const updateSalesOrder = (req, res, next) => {
    ...
    db.none('update sales_order set customer_id=$1, customer_name=$2, account_id=$3, sales_order_status_id=$4 where sales_order_id=$5',
            fieldsToUpdate)
        .then(() => {
            res.status(200)
                .json({
...
export const getSoldProductsBySalesOrder = (req, res, next) => {
    ...
    db.any('select sold_product_id, product.product_id, description, price, quantity, total' +
            ' from sold_product' +
            ' join product' +
            ' on sold_product.product_id = product.product_id' +
            ' where sold_product.sales_order_id = $1', salesOrderID)
        .then(data => {
            res.status(200)
                .json({
                    status: 'success',
...
export const createSoldProduct = (req, res, next) => {
    ...
    db.none('insert into sold_product(product_id, sales_order_id, quantity)' +
            ' values(${product_id}, ${sales_order_id}, ${quantity})', req.body)
        .then(() => {
            res.status(200)
                .json({
                    status: 'success',
...
export const updateSoldProduct = (req, res, next) => {
    ...
    db.none('update sold_product set quantity = $1 where sold_product_id=$2', 
        [req.body.quantity, soldProductID])
        .then(() => {
            res.status(200)
                .json({
                    status: 'success',
